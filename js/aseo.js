...
import { initializeGapiClient, appendData } from '../api/googleSheets.js';

document.addEventListener("DOMContentLoaded", async () => {
    await initializeGapiClient();

    // Formulario de asignación de zona
    document.getElementById("zone-assignment-form").addEventListener("submit", async (e) => {
        e.preventDefault();
        ...
        // Valores para la asignación en Google Sheets
        const values = [[cleaner, zone, breakTime, dateTime]];
        await appendData("cleaner!V2:Z", values);

        // Confirmación visual de éxito
        Swal.fire({
            title: 'Asignación realizada',
            text: `Zona asignada a ${cleaner}.`,
            icon: 'success',
            confirmButtonText: 'OK'
        });

        e.target.reset(); // Limpiar el formulario tras enviar
    });
});
